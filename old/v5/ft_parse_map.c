...
void   ft_parse_map(t_env *e, char **argv)
{
    ...
    if (!(e->map = (t_map*)malloc(sizeof(t_map))) ||
		!(e->map->lines = (t_line**)malloc(sizeof(t_line *) * ft_count_row(argv[1]))) ||
		!(e->map_buffer = (t_map*)malloc(sizeof(t_map))) ||
		!(e->map_buffer->lines = (t_line**)malloc(sizeof(t_line *) * ft_count_row(argv[1]))))
    {
		ft_putstr("malloc failed");
		exit(EXIT_FAILURE);
	}
